...
- Usa `next()` para pasar al handler; nunca mutes `Astro.locals` globalmente

### Scripts del cliente (`scripts/`)
- `mf-loader.ts`: usa `loadMultipleMicrofrontends` con `Promise.allSettled` — nunca falla en silencio
- `mf-runtime.ts`: `importRemoteWithTimeout` con timeout 8 s y whitelist de orígenes
...
## Patrones de Carga MFE

```typescript
// mf-loader.ts — carga paralela con manejo de fallos individuales
const results = await Promise.allSettled([
  importRemoteWithTimeout('http://localhost:5173/assets/mf-manifest.json', 8000),
  importRemoteWithTimeout('http://localhost:4201/assets/mf-manifest.json', 8000),
]);

results.forEach((result, index) => {
  if (result.status === 'rejected') {
    console.error(`MFE ${index} failed to load:`, result.reason);
    // Mostrar fallback, nunca romper la página
  }
});
```
