...
```typescript
// Ejemplo de Custom Element correcto
export class PortfolioAngularMf extends HTMLElement {
  private destroyRef = inject(DestroyRef);

  connectedCallback() {
    EventBus.on('portfolio:updated', this.handleUpdate, { signal: this.destroyRef.onDestroy() });
  }

  disconnectedCallback() {
    // El DestroyRef limpia automáticamente vía signal
  }
}
```
...
│   │   ├── infrastructure/  → adapters EventBus, HTTP
│   │   └── presentation/    → componentes, pipes, directivas
